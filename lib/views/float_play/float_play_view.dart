...
class _FloatPlayViewState extends State<FloatPlayView>
  ...
  final logic = Get.find<FloatPlayLogic>();
  double _containerWidth = 170;
  bool _isExpanded = true;
  Timer? _hideTimer;
  late Worker _spinWorker;
  ...
  void initState() {
    ...
    print('AnimationController is being init at: ${DateTime.now()}');
    ...
    _spinWorker = ever(logic.state.isPlay, (bool spinning) {
      if (mounted) {
        if (spinning) {
  ...
  void _onTick() {
    setState(() {
      logic.state.posY.value =
          _filingController.value.clamp(70.0, screenHeight);
    });
  }
  ...
  void dispose() {
    ...
    _spinWorker.dispose();
    print('AnimationController is being disposed at: ${DateTime.now()}');

    _filingController.dispose();
    _hideTimer?.cancel();
    super.dispose();
  }
              ...
              child: GestureDetector(
                ...
                onVerticalDragUpdate: (details) {
                  setState(() {
                    ...
                    logic.state.posY.value =
                        logic.state.posY.value.clamp(70.0, screenHeight);
                  });
                },
